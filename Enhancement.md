...
This file describes the feature in this codebase (Java / Spring Boot / 3-tier layered). Sibling files in `AyvalikBankHA-JAVA`, `AyvalikBankHA-NET`, `AyvalikBankLA-NET`, `AyvalikBankHA-Python`, `AyvalikBankLA-Python` describe the same feature in their respective stacks so the impact can be compared side by side.
